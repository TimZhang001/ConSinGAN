...
mvtectAD = "/home/zhangss/PHDPaper/06_ConSinGAN/mvtecAD/"
...
def generate_samples(netG, reals_shapes, noise_amp, scale_w=1.0, scale_h=1.0, reconstruct=False, n=50):
    if reconstruct:
        ...
        if opt.train_mode == "generation" or opt.train_mode == "retarget":
            functions.save_image('{}/reconstruction.png'.format(dir2save), reconstruction.detach())
            functions.save_image('{}/real_image.png'.format(dir2save), reals[-1].detach())
        elif opt.train_mode == "harmonization" or opt.train_mode == "editing":
            functions.save_image('{}/{}_wo_mask.png'.format(dir2save, _name), reconstruction.detach())
            functions.save_image('{}/real_image.png'.format(dir2save), imresize_to_shape(real, reals_shapes[-1][2:], opt).detach())
        return reconstruction

    if scale_w == 1. and scale_h == 1.:
        dir2save_parent = os.path.join(dir2save, "image")
    else:
    ...
    for idx in range(n):
        ...
        functions.save_image('{}/gen_sample_{:04d}.png'.format(dir2save_parent, idx), sample.detach())


if __name__ == '__main__':
    parser = get_arguments()
    parser.add_argument('--model_dir',   help='input image name', default='/home/zhangss/PHDPaper/06_ConSinGAN/mvtecAD/grid/metal_contamination')
    parser.add_argument('--gpu',         type=int, help='which GPU', default=0)
    parser.add_argument('--num_samples', type=int, help='which GPU', default=1000)
    parser.add_argument('--naive_img',   help='naive input image  (harmonization or editing)', default=None)

    opt = parser.parse_args()

    object_names  = os.listdir(mvtectAD)
    
    for object in object_names: 
        object_path = os.path.join(mvtectAD, object)
        sub_names   = os.listdir(object_path)
        for sub_name in sub_names:

            opt.model_dir = os.path.join(object_path, sub_name)
            ...
            dir2save = os.path.join(opt.model_dir, "sample_eval")
            make_dir(dir2save)
            gen_image_path = os.path.join(dir2save, "image", "gen_sample_0999.png")
            if os.path.exists(gen_image_path):
                print("Samples already generated. Skipping...")
                continue
            ...
            if opt.train_mode == "generation" or opt.train_mode == "retarget":
                ...
                if os.path.exists(mask_file_name):
                    ...
                    functions.save_image('{}/{}_w_mask.png'.format(dir2save, _name), out.detach())
                else:
